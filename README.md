...
Autenticação JWT: Sistema de Login (/Login) para geração de token de acesso.

Controle de Acesso: Proteção dos endpoints de produto com [Authorize].

Banco de Dados Local: Utiliza SQLite e Entity Framework Core para persistência de dados.

Swagger/OpenAPI: Documentação interativa para testar os endpoints.
...
Pacotes Principais (vistos no .csproj e classes):
...
Uma IDE (Visual Studio, VS Code, Rider, etc.) é recomendada.
